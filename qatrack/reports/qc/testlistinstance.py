...
class TestListInstanceDetailsReport(BaseReport):
    ...
    template = "reports/qc/testlistinstance_details.html"  #!!!!!!!!!!!!!!!!!!!!!!!!!!! change back to standard
...
class RoutineQAReport(BaseReport):
    ...
    filter_class = filters.TestListInstanceByUTCFilter
    #filter_class = filters.TestListInstance_forroutineqa_Filter
    description = mark_safe(
        _l(
            "Example report to recreate terabase output"
        )
    )
    ...
    template = "reports/qc/Routine_qa_report.html" 
    __test__ = False  # supress pytest warning
    ...
    def get_context(self):
        ...
        if len(qs) > 1:
            ...
            qs_filt = []
            for tli in qs:
                if not tli.test_list.name in change:
                    change.append(tli.test_list.name)
                    qs_filt.append(tli)
                else: 
                    pass
            context['queryset'] = qs_filt
        else:
            context['queryset'] = qs
    ...
    def to_table(self, context):
        ...
        for tli in list(context['queryset'])[-1:]:
